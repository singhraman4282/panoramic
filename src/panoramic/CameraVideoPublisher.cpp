  ...
  camera_calib_service_(0)
{
  ...
  do {
    cv::VideoCapture t_cap(camera_video_capture_.size()-1);
    camera_limit_reached = !t_cap.isOpened();
  ...
  for(int i = 0; i < camera_video_capture_.size(); i++) {
    ...
    ros::Publisher t_pub = nh_.advertise<sensor_msgs::Image>(camera_name.c_str(), 1);
