...
int main(int argc, char** argv)
{
  ...
  test_stitch.request.phi_res = test_stitch.request.theta_res = panoramic::SphericalStitchRequest::MAX_RES;
  ...
  sensor_msgs::Image first_im, second_im;
  cv_bridge::CvImage cvi_first( std_msgs::Header(), std::string("brg8"), first_image );
  cv_bridge::CvImage cvi_second( std_msgs::Header(), std::string("brg8"), second_image );
  ...
  if(p_client.call( test_stitch )) {
    ...
    ROS_ERROR("Could not call the panoramic server.");
  }
